...
function showPhoto(id, url, author_id, album, date, start){
    ...
    document.getElementById('setAsMainPhoto').setAttribute('onclick','updateMainPhoto('+idUser+','+id+');');
    if (author_id != idUser){
        $('#setAsMainPhoto').hide();
        $('#deletePhoto').hide();
    }else{
...
function loadPhotos(idAlbum, start, count){
    $.getJSON(photoUrl,
        {
            idAlbum:idAlbum,
            start:start,
        ...
        function(data){
            if (count == 1){
                ...
                document.getElementById('setAsMainPhoto').setAttribute('onclick','updateMainPhoto('+data[0].album.user.id+','+data[0].id+');');
                if (data[0].album.user.id != idUser){
                    $('#setAsMainPhoto').hide();
                    $('#deletePhoto').hide();
                }else{
                    $('#setAsMainPhoto').show();
...
function updateMainPhoto(idUser,idPhoto){
    $.post(updatePhotoUrl,
        {
            idUser: idUser,
            idPhoto: idPhoto,
